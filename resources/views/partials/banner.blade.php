@isset($banner)
    ...
    @elseif($banner->type === 'split_bg')
        <a href="{{ $banner->link }}" class="banner split-bg" style="display:flex;align-items:center;margin:20px 0;text-decoration:none;">
            <div style="flex:1;padding:20px;background:#4a90e2;color:#fff;">
                <h2>{{ $banner->title }}</h2>
                <p>{{ $banner->subtitle }}</p>
                <span class="btn" style="margin-top:12px;background:#fff;color:#000;padding:8px 16px;border-radius:6px;display:inline-block">Подробнее</span>
            </div>
            <div style="flex:1;">
